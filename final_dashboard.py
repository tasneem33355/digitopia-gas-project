...
[data-testid="stAppViewContainer"] {
    background: linear-gradient(135deg, #000000 30%, #0B3D91 100%); /* أسود إلى أزرق غامق */
...
[data-testid="stSidebar"] {
    background-color: #0B3D91; /* أزرق غامق */
    color: white;
}
h1, h2, h3, h4 {
    color: gold;  /* العناوين تظل ذهبية */
    font-weight: bold;
}
...
def load_model():
    try:
        ...
        if expected_features != actual_features:
            # Add dummy features to match model expectations
...
with st.sidebar:
    ...
    st.markdown("""
<style>
/* All sidebar buttons colored very dark blue */
[data-testid="stSidebar"] button, 
[data-testid="stSidebar"] div.stButton > button {
    background-color: #041C32;  /* كحلي أغمق جداً */
    color: #FDF5E6;             /* نص فاتح للوضوح */
    border: none;
    border-radius: 5px;
...
[data-testid="stSidebar"] div.stButton > button:hover {
    background-color: #0B3D91;  /* كحلي داكن عند المرور */
    color: #FDF5E6;
}
</style>
...
""", unsafe_allow_html=True)
    ...
    if os.path.exists("shared_state.json"):
        ...
        try:
            ...
            if is_fresh:
                st.success(f"✅ Data is fresh")
                st.write(f"Points: {len(state_data.get('data_buffer', []))}")
            else:
                st.warning("⚠️ Data might be stale")
        except:
            st.warning("⚠️ Could not check data freshness")
    else:
        st.error("❌ No shared file found")
